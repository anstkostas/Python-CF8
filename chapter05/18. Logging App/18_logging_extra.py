...
def configure_logger(log_file: str, logger_name: str) -> logging.Logger:
    ...
    # From -> https://codeshare.io/anw4ZV
    ...
    # From -> https://codeshare.io/GbprEM 
    # Mode       | Purpose          | Description
    # ---------- | ---------------- | -------------------------------------------------
    # 'a'        | Append           | Opens the file for appending. Logs are added at the end of the file
    ...
    # 'r+'       | Read/Write       | Opens the file for both reading and writing. The file pointer is at 
    #            |                  | the beginning. Raises an error if the file doesn’t exist.
    # 'a+'       | Append/Read      | Opens the file for appending and reading. The file pointer is at the 
    #            |                  | end for writing, but you can read the content as well. 
    #            |                  | Creates the file if it doesn’t exist.
    # 'w+'       | Write/Read       | Opens the file for both writing and reading. Overwrites the file 
    #            |                  | if it exists; creates a new file if it doesn’t.
    # 'x+'       | Create/Read/Write| Creates a new file for both reading and writing. Raises an error 
    #            |                  | if the file already exists.
    # 'b'        | Binary Mode      | Opens the file in binary mode. Commonly used with other modes 
    #            |                  | like 'rb', 'wb', or 'ab' for binary read/write/append.
    # 't'        | Text Mode        | Opens the file in text mode. This is the default mode and often 
    #            |                  | used implicitly.
...
def main():
    ...
    employee_to_find2 = "Bob"

    try:
        # Attempt to find the index of employee_to_find in employee_names
        # Attempts that dont throw errors must be called first, else this try..except will stop executing.
        index2 = search_item(employee_names, employee_to_find2, logger)
        index = search_item(employee_names, employee_to_find, logger)
        print(f"Employee '{employee_to_find}' found at index {index}.")
        print(f"Employee '{employee_to_find2}' found at index {index2}.")
    except ValueError:
        print(f"Employee '{employee_to_find}' was not found in the list.")
        print(f"Employee '{employee_to_find2}' was not found in the list.")
